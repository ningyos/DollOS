# BridgeSupervisor — Discord Bridge Internalization (Option A) — Design
...
| **`[bridge]` 5 個 restart 旋鈕＝config bloat** | 直接違反使用者「`[bridge]` 只放指標＋enable、別讓 daemon config 變肥」的明文指示；對 localhost 乖巧子行程是把 systemd `StartLimit*` 語意搬進 user-facing config 重造。 | **降為 module-level 常數**（`bridge_supervisor.py`）。`[bridge]` **只留 `enabled` + `config`**。`retention_days` 也移出（它是 bridge 行為，屬 bridge.toml — §4/§9-4）。真要可調，等實際遇到 crash-loop 再加。 | §3.1, §4 |

外加已修的 lifecycle-safety 硬傷（Review 1 I3/I4/M6/M7、Review 2 M1）：

- **respawn 競態**（I3）：shutdown 期間 stop() 讀到 stale `self._proc`，剛 fork 出的新 bridge 逃過 kill → 孤兒。修：`_spawn` fork 回來後**先檢查 `_stopping` 再原子指派 `self._proc`**，且 stop() 於 cancel task **後**再讀一次 `self._proc` 補刀（§3.3、§3.5）。
- **`_spawn()` OSError／supervise 迴圈裸奔**（I4 + M1）：`create_subprocess_exec` 在高負載丟 `OSError(EAGAIN/ENOMEM)`，或迴圈本體任何非預期例外，會讓 supervise task 靜默死掉、bridge 從此無人看顧。修：**整個迴圈本體包 try**，非預期例外當一次 crash 走 backoff+retry，不逃出迴圈（§3.4）。
...
- **start() idempotency guard**（M7）：照抄 `system_pulse.py:341-342` 的 `if self._task is not None: return`，避免被呼叫兩次 → 兩個 supervise loop → 兩個 bridge（§3.4）。
...
既有的 WS 連線（本來就是 reconnect-based、已隔離）。
...
**In scope:**
- 新 `BridgeSupervisor` class：spawn（含 `PR_SET_PDEATHSIG` 反孤兒）/ supervise（crash 偵測 +
  backoff 重啟 + crash-loop 上限 + healthy-uptime 重置 + 迴圈全體例外防護）/ graceful terminate
  （**SIGINT**→wait→SIGKILL→reap）。
...
- kernel 接線（`__init__` 建構、server 起來後 `start()`、inner finally 早停 + outer finally `stop()`）。
...
## 3. BridgeSupervisor 設計

新模組 `src/dollos/bridge_supervisor.py`，一個長生命週期單一子行程的看顧者。結構上最接近的既有樣板是
`src/dollos/perception/system_pulse.py:337-353`（單一 `self._task`、`start()` 建 task 且有
idempotency guard @341-342、`stop()` cancel + await），子行程管理的原語則沿用 `MonitorRunner` /
`ShellRunner`（投查二）。

### 3.1 建構與追蹤狀態

在 `DollOSKernel.__init__` 內、緊鄰 `shell_runner` / `monitor_runner` 建構處
（`kernel.py:422-430`）建立。建構參數**只**從 `settings.bridge`（{enabled, config}）與環境推導；
**restart 旋鈕已降為模組常數**（§0 收斂點四）：

```python
# bridge_supervisor.py — module-level 常數（非 user-facing config；YAGNI，要調再升 config）
...
_RETENTION_DAYS = 30       # bridge ambient-log 保留天數（傳給 --retention-days；見 §4/§9-4）

self.bridge_supervisor = BridgeSupervisor(
    enabled=settings.bridge.enabled,
    python=sys.executable,               # 跟 units.resolve_params 同慣例 (units.py:108)
    bridge_config=settings.bridge.config,  # Path | None
    data_root=settings.data.root,          # 與 daemon 同一份 data/（投查三 (d)）
    daemon_ws=_derive_daemon_ws(settings.ipc),  # 見 §3.2
    on_gave_up=self._emit_bridge_down_perception,  # §8 可見性 / §9-3
)
```

追蹤欄位（極簡，單行程所以不需 dataclass registry）：
...
### 3.2 argv 與 daemon WS 推導（投查三）

spawn 用 `asyncio.create_subprocess_exec`（**argv list，非 shell**）—— 刻意不走
`create_subprocess_shell`（不同於 monitor/shell 跑任意字串指令的用途），因為 argv 固定，走 exec 少一層
shell、且**杜絕 config 路徑被 shell 解讀**（見 §8 攻擊面）：

```python
argv = [
    self._python, "-m", "dollos.discord_bridge",
    "--daemon", self._daemon_ws,
    "--config", str(self._bridge_config.expanduser().resolve()),
    "--data-root", str(self._data_root.expanduser().resolve()),
    "--retention-days", str(_RETENTION_DAYS),
]
```

只有 `--config` 是 bridge 端 required（`__main__.py:74-79`）。路徑全部 `.resolve()` 成絕對路徑，
所以子行程繼承 daemon 的 cwd 也不影響。`--data-root` **必須**與 daemon 同一份 `data/`
（投查三 (d)：ambient Discord 語料寫到 `{data-root}/discord/…`，錯了會寫到別棵樹）。因為 argv 是
`sys.executable -m dollos.discord_bridge`（**直接子行程、無 `uv run` 中介**），§3.3 的 PDEATHSIG
監看的正好是 bridge 本身，不是某個 wrapper。
...
### 3.3 spawn 原語（投查二 (a)(b)）＋反孤兒 PDEATHSIG（§0 收斂點二）

```python
def _preexec() -> None:
    # child 內、exec 之前跑。Linux-only（本專案就 Linux）。
    os.setsid()                                    # 獨立 process group（見下）
    _prctl(PR_SET_PDEATHSIG, signal.SIGINT)        # daemon 一死 → kernel 對 bridge 發 SIGINT
...
async def _spawn(self) -> None:
    ...
    if self._stopping:                             # (I3) stop() 在我們 fork 期間立了旗
        proc.kill(); await proc.wait()             # 這個新 bridge 沒被存進 _proc，就地 reap
        raise _SpawnAborted                        # supervise 迴圈據此跳出（不算 crash）
    self._proc = proc                              # 原子指派（此行前後無 await）
    logger.info("bridge spawned pid=%d argv=%s", proc.pid, argv)  # 可見性（§8）
```
...
- **不再用 `start_new_session=True` 參數**、改在 `_preexec` 內顯式 `os.setsid()`：因為我們已經有
  `preexec_fn`，把 setsid 也收進去（順序：setsid → prctl → getppid 檢查），語意等價於原
  `start_new_session=True`（獨立 process-group leader，killpg 能連未來孫行程一起收），
  與 `monitor_runner.py:176` / `shell_runner.py:94` 同精神。
- argv **不含 token**（只含 bridge.toml 的**路徑**），所以 `ps` / journal 都看不到密鑰（§8）。
- **`_spawn` 失敗（`OSError` 等）不在此吞**：交給 §3.4 的迴圈全體 try 當一次 crash 處理（I4）。

### 3.4 supervise 迴圈：crash 偵測 + backoff 重啟 + crash-loop 上限（＋全體例外防護）

`start()` 先做 idempotency guard（**M7**，照抄 `system_pulse.py:341-342`）：

```python
def start(self) -> None:
    if not self._enabled:
        logger.info("bridge disabled by config"); return   # no-op，零開銷
    if self._task is not None:
        return                                              # (M7) 已在跑，別開第二個 loop
    if self._bridge_config is None or not self._bridge_config.exists():
        logger.error("bridge enabled but config missing (%s) — not spawning",
                     self._bridge_config)                   # fail-fast，清楚訊號
        return
    self._task = asyncio.create_task(self._supervise(), name="bridge-supervisor")
```

supervise 迴圈 —— **整個迴圈本體包在 try 內**（I4 + Review 2 M1）：`_spawn()` 的 `OSError`、
...
async def _supervise(self) -> None:
    while not self._stopping:
        ...
        try:
            await self._spawn()                     # §3.3（含 _SpawnAborted）
            rc = await self._proc.wait()            # 阻塞到 bridge 死掉（reap 也在此完成）
            ...
            logger.exception("bridge spawn/wait failed — treating as crash")
            rc = None
        uptime = monotonic() - started_at

        if self._stopping:                          # (A) killed-by-us on shutdown → 不重啟
            break
        if rc == 0:                                 # (B) clean self-exit（含 SIGINT graceful）
            logger.warning("bridge exited cleanly (rc=0) — not restarting")
            break
        # (C) crash（含 startup config error、fatal-token 退出 §3.4a、spawn OSError）
        if uptime >= _HEALTHY_UPTIME_S:
            self._consecutive_failures = 0          # 撐夠久才死 → 不是 crash-loop，重置
        self._consecutive_failures += 1
        if self._consecutive_failures > _MAX_CONSECUTIVE:
            logger.error(
                "bridge crashed %d× consecutively (rc=%s) — giving up until daemon restart",
                self._consecutive_failures, rc)
            self._on_gave_up(rc)                    # (M8) 發 perception 給 Doll（§8/§9-3）
            break                                   # crash-loop 上限 → 放棄並大聲浮現
        delay = min(_BACKOFF_BASE_S * 2 ** (self._consecutive_failures - 1), _BACKOFF_MAX_S)
        logger.warning("bridge crashed (rc=%s, uptime=%.1fs) — restart in %.1fs (fail #%d)",
                       rc, uptime, delay, self._consecutive_failures)
        try:
...
| 狀況 | rc / 訊號 | 動作 |
|---|---|---|
| **killed-by-us**（shutdown SIGINT，§3.5） | `_stopping==True` | 不重啟，跳出 |
...
（此改動屬 Task B2，明列在 §9-1 讓使用者拍板「現在就做」vs「v1 出 watchdog-only 並文件標明盲區」。**傾向：現在做**——否則 headline 承諾空頭。）

### 3.5 graceful terminate（**SIGINT**→wait→SIGKILL→reap）——**net-new pattern**
...
rc=0、grace_s 也**真正有意義**（bridge 若沒在 grace 內乾淨收，才升級 SIGKILL）。這是本設計**新增**的原語：

```python
async def stop(self) -> None:
    self._stopping = True                        # 先立旗：supervise 迴圈醒來即不重啟；且擋 §3.3 respawn 競態
    proc = self._proc
    ...
    # (I3) cancel task 後再讀一次：若 stop 期間 supervise 剛好 fork 了新 bridge，補刀
    late = self._proc
...
- 因為 bridge 自成 session（§3.3 `os.setsid()`），一律 `os.killpg(os.getpgid(pid), …)` 對**整組**
...
- **idempotent**：`_proc is None` 或已退出時，kill 段整段跳過；可被安全呼叫多次（§3.6 需要 —— inner
  finally 早停 + outer finally backstop 會呼叫兩次）。
- **respawn 競態雙保險**（I3）：`_stopping` 先立旗讓 §3.3 `_spawn` 中途 abort；末尾再讀一次
  `self._proc` 補刀 fork-在-cancel-窗口的漏網新行程。

### 3.6 kernel 接線點（投查一）

**spawn hook：** `await self.server.start()`（`kernel.py:1032`）**回傳後**呼叫
`self.bridge_supervisor.start()`。`server.start()` 一回傳，WS 就真的在收連線
...
spawn（省掉一次白費的 failed-connect）。bridge 是**子行程不是 asyncio task**，所以必須在 1032 之後
spawn（可放進背景 task 群組 `kernel.py:1061-1073` 一起起）。
...
1. **inner finally 早停（`kernel.py:1102-1105` runner 群組旁）——「做」而非選配（M6）。**
   這是**常態 graceful 路徑**：daemon 關機時 mind loop 已起、走到 inner finally，此處 SIGINT bridge，
   讓它**趁 daemon 還大致健在時乾淨關 gateway**。否則 `server.stop()`（1081）先斷了 WS，bridge 會在
   1081→1156 這數秒 teardown（memsearch.close、keeper/skeptic 收尾）期間每 5s 敲一個正在死的 daemon。
...
> `stop()` idempotent 使雙呼叫安全。**加上 §3.3 的 PDEATHSIG**，連 daemon 被 SIGKILL（跑不到任何
...
restart 旋鈕已降為 §3.1 的模組常數；`retention_days` 亦移出——見 §9-4）。鏡射 `SystemPulseConfig`
...
| 事件 | 偵測 | supervisor 反應 |
|---|---|---|
| daemon 啟動、`enabled=true` | `start()` after `server.start()` | spawn bridge（含 PDEATHSIG），開始看顧 |
| daemon 啟動、`enabled=false` | `start()` | no-op（記一行、零開銷） |
| bridge 崩潰（rc≠0 / spawn 例外） | `proc.wait()` 回傳或 §3.4 try 攔到、`_stopping==False` | backoff 重啟；連續超上限 → 放棄 + error + **perception** |
| **fatal 連線錯誤（壞 token，§3.4a）** | bridge **非零退出**（不再 hang/無限重試） | 走 crash 路徑 → backoff → 超上限放棄（config typo 場景正確） |
| bridge 乾淨自退（rc==0） | 同上、rc==0 | 不重啟、記 warning（自主下幾乎不發生） |
| daemon 關機（graceful） | inner finally 早停 + outer finally backstop | **SIGINT**→wait `_GRACE_S`→SIGKILL→reap；不重啟 |
| **daemon 被 SIGKILL（跑不了 finally）** | kernel 發 **PDEATHSIG**（§3.3） | bridge 收 SIGINT 自盡 → **不留孤兒、無雙 bridge** |
| bridge 撐過 `_HEALTHY_UPTIME_S` 才死 | uptime 判斷 | 重置 failure counter（非 crash-loop） |
...
迴圈 / shell 的 `communicate()`）。bridge 沒有那個 drain task，所以 inherit 是零死鎖、零額外程式碼、
且與今天 standalone bridge 的行為一致（bridge → journal）的最低風險選擇。
...
supervisor 發訊號/IPC 命令，讓它 cycle 子行程重讀 bridge.toml）——列後續，v1 不做，但設計裡承認缺口。
...
改動只碰**錯誤分類與退出碼**，同樣不觸碰 isolation 邊界。

**孤兒 / zombie 防禦（三層，§0 收斂點二）。**
- **PDEATHSIG（§3.3）—— 主防線**：daemon 一死（**含 `kill -9`**）→ kernel 對 bridge 發 SIGINT →
  bridge 自盡，**發生在新 daemon 起來前 → 根本不給雙 bridge 窗口**。手動跑 daemon（無 systemd cgroup
...
- **殘存邊界（已大幅縮小）**：PDEATHSIG 唯一漏接的窗口是「fork 完成、`_prctl` 尚未執行、daemon 恰在此
  微秒內死」——由 `_preexec` 的 `getppid()==1` 自盡守住（§3.3）。理論上仍有「preexec 全跑完前 daemon
  死」的極窄縫，此時 **pidfile（§9-1，選配 Task E）** 是最後一道：孤兒持有 `bridge.pid`，操作者/新
...
**可見性（弱機制 playbook 慣例，三面向）。** 每次 spawn 記 `pid + argv`（INFO）；每次退出記 `rc +
uptime + 重啟決策`；**放棄（終局事件）記 error 並發一個 perception 給 Doll**（`on_gave_up`，§3.1/§3.4）
—— 讓 Doll/操作者知道「Discord 在線整個掛了」，不再靜默（M8；perception 內容見 §9-3）。稀有／關鍵的
看顧行為對操作者三面向可見。
...
bridge.toml 路徑來自 daemon config（操作者所有，非 Doll 經 tool 可寫的 data/ 內容）。
...
2. **反孤兒主手段：`PR_SET_PDEATHSIG`（本版已採為核心）＋ bridge-side pidfile（選配 Task E）。**
   ...
   PDEATHSIG 的極窄理論競態。**傾向：PDEATHSIG 為核心（已納入 §3.3）、pidfile 為選配額外防線。**
...
4. **`retention_days` 放哪？（本版已移出 daemon config）** 它是 bridge 的 ambient-log 行為
   （`__main__.py:88-91`），本版讓 supervisor 傳模組常數 `_RETENTION_DAYS=30`。若操作者要可調，
...
5. **要不要補 `dollosctl reload-bridge`（熱載 bridge.toml、不重啟 daemon）？** 補回單服務化失去的
   bridge-only restart 能力（§7 I5）。**傾向：v1 不做**（記缺口）；真的常改 token/owner 再補。**← 確認。**
...
- **Task A — `[bridge]` config schema（最小）。** `config.py` 加 `BridgeConfig`（**只有 `enabled` +
  ...
  旋鈕、無 retention_days**（那些是 §3.1 模組常數）。純 config、無行為。測試：預設關、缺 config 的
  enabled 報錯、extra key 被 forbid（含舊的 restart 旋鈕 key 被拒，防遺留）。
- **Task B — `BridgeSupervisor` class（核心）。** `bridge_supervisor.py`：模組常數 + spawn（argv/WS
  推導 + **`_preexec`：setsid + `PR_SET_PDEATHSIG(SIGINT)` + `getppid()==1` 自盡** + fork-後 `_stopping`
  recheck 原子指派）+ supervise 迴圈（**全體 try 防護**、三態 + backoff + healthy-uptime 重置 +
  crash-loop 上限 + `on_gave_up`）+ `start()` **idempotency guard**（M7）+ graceful `stop()`
  （**SIGINT**→wait→SIGKILL→reap、idempotent、末尾 re-read `_proc` 補刀）。用**假子行程**測
  （spawn 短命 `python -c` script 觸發：乾淨退出／崩潰／撐久後崩／SIGINT-graceful／SIGINT-逾時-SIGKILL／
  spawn OSError（monkeypatch）／shutdown-中途-fork 競態）。**PDEATHSIG live-assert**：spawn 子行程 →
  `kill -9` 掉「假父」→ `pgrep` 確認子行程隨之消失。斷言：group-kill + reap（無 zombie）、backoff 序列、
  超上限放棄且觸發 `on_gave_up`、`enabled=false` 時 `start()` no-op、double-`start()` 只一個 loop。
  完全不碰真 Discord。
- **Task B2 —（繫於開放決策 1）bridge fatal-error 分類（§3.4a，surgical）。** `discord_bridge/__main__.py`
  ...
  獨立成一支好審查/好 veto。
- **Task C — kernel 接線。** `__init__` 建構（§3.1）、`server.start()` 後 `start()`（§3.6）、
  **inner finally（`kernel.py:1102-1105`）早停 + outer finally（`kernel.py:1156`）backstop 兩個
  `stop()` 呼叫點**。derive WS URL / argv from settings。Live-smoke：真 daemon + 真 bridge.toml + 私人
  測試伺服器，看 bridge 被帶起、daemon graceful 關機時乾淨收掉（gateway 有優雅斷）、**`kill -9` daemon
  後 `pgrep -f dollos.discord_bridge` 為空**（PDEATHSIG 驗證）、無孤兒。
...
- **Task F — 文件收尾。** `CLAUDE.md` 架構段記「daemon 內化 bridge（PDEATHSIG 反孤兒、SIGINT graceful、
  fatal-token 會退出）」、`roadmap.md` 加這步、既有安裝遷移註記（升級須重跑 install）、
  `config.example.toml` 的 `[bridge]`（兩行）範例。

依序：**Task A → B → (B2) → C → D**（config 先、supervisor 次、bridge 分類三〔若採納〕、接線四、ctl 五）；
