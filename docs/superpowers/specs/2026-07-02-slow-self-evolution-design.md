...
- **Evolution events**: `{ts, kind: "evo_candidate" | "evo_counter" | "evo_adopt" | "evo_reject" | "evo_expire" | "evo_kill" | "evo_no_change" | "evo_error" | "evo_repair" | "external_edit", text?, old_text?, rationale?, reason?, drift_score?}` (`evo_repair` = the §5 crash-repair notice; audit-only, never a decision event; `drift_score` = 1 − jaccard, dissimilarity, first adoption `null`).
...
**High-water mark (HWM)** := byte offset into `self_history.jsonl`. Captured when the driver snapshots the file; **committed to MindState only when the pass completes with a verdicted outcome** (`evo_no_change`, `evo_kill`, or candidate creation). Not committed on cancel or error. **Restored on `evo_expire`** (`pending.json` carries `hwm_before`; expiry writes it back): evidence Doll never got to judge re-seeds the next pass — a weak model failing to call the tool must not silently consume weeks of evidence (R2). `evo_reject` does NOT restore — her verdict consumed the evidence. **Diary-anchor asymmetry (Plan-3 adjudication):** the diary clause anchors to `last_evolution_attempt_at`, which advances on every decision event including expire — expired-candidate diary evidence therefore does NOT re-seed the way pins do. Consciously accepted: the material gate is an OR, pins are the primary evidence channel and their HWM restoration alone re-seeds the pass, and the next diary day re-fires the clause naturally; a separate restorable diary anchor was judged schema churn without proportionate value.
...
- **Echo-equivalence test (R2 — load-bearing, defined here; amended at Plan-2 implementation):** normalize = strip surfacing markers + Unicode NFKC + **remove all punctuation + remove all whitespace** (amendment rationale: for CJK prose, stray internal spaces/punctuation in an echo are noise — the original "whitespace collapse" definition made a space-jittered echo fail BOTH the exact branch and jieba tokenization (0.33 similarity), misrouting an intended adopt into 送審, the exact weak-model failure this test exists to prevent; false-positive equivalence is the safe direction since it adopts the skeptic-passed candidate verbatim). `text` is *equivalent* to a reference if normalized-exact-equal OR jieba-Jaccard similarity ≥ 0.9 on the marker-stripped texts.
...
- **Tamper tripwire + ratification (transition-gated — R2).** The last sanctioned text = the latest `evo_adopt`'s `text` (bootstrap: none → any non-empty file counts as an edit; "restore" then means delete). At render time, if file ≠ sanctioned:
  ...
  - **Unchanged divergent file** (text == the already-logged edit): no side effects — render sanctioned text and move on (no per-turn log spam). **Bounded completion rule (Plan-2 R3″):** if no slot exists while the file still equals the logged edit (the edit's slot creation crashed after the log line), complete it idempotently — re-run mechanical checks; pass → create the external slot; fail → plain restore with no duplicate `external_edit` line. Cannot livelock: fires only while that exact logged text persists with no slot. Relatedly, every tripwire branch is **log-before-mutate** (§3.2 ordering philosophy) and the tripwire call is never wrapped in a swallow — an audit-append failure aborts the turn loudly, leaves the file divergent, and retries next turn.
